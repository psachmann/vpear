// <copyright file="DeviceStatusChangedHandlerTest.cs" company="Patrick Sachmann">
...
{
    public class DeviceStatusChangedHandlerTest
    {
        [Theory]
        [InlineData(0)]
        [InlineData(100)]
        public async Task UpdatePollFramesJobTest(int frequency)
        {
            ...
            var logger = Mocks.MockLogger<DeviceStatusChangedHandler>();
            var handler = new DeviceStatusChangedHandler(factory, logger.Object);
            ...
            await handler.Handle(new DeviceStatusChangedEvent(device), default);
